...
EAPI void
_edje_pick_context_set(Edje_Pick *c)
{
   context = c;
}
...
EAPI const char *
_edje_pick_err_str_get(Edje_Pick_Status s)
{
   const char *err = NULL;
      ...
      const char *errfile, const char *errgroup)
{
   ...
   if (_edje_pick_err_str_get(s))
     EINA_LOG_ERR("%s\n", _edje_pick_err_str_get(s));
...
EAPI int
_edje_pick_process(int argc, char **argv)
{
